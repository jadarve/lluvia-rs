...
#[test]
fn test_image_downsample_y() -> Result<()> {
    ...
    let builder_down = session.load_compute_node_builder("lluvia/imgproc/ImageDownsampleY_r8ui")?;
    ...
    let staging_out = session.create_buffer_host_visible((width * out_height) as u64)?;
    ...
    for y in 0..out_height {
        for x in 0..width {
            let img_0 = input_data[(2 * y * width + x) as usize] as u32;
            let img_m = if y == 0 {
                img_0
            } else {
                input_data[((2 * y - 1) * width + x) as usize] as u32
            };
            let img_p = if y == out_height - 1 {
                img_0
            } else {
                input_data[((2 * y + 1) * width + x) as usize] as u32
            };
            ...
            let actual = output_data[(y * width + x) as usize] as u32;
